...
def findposition(badpart, sourcecode):
    ...
    while not end:
        ...
        if sourcecode[pos] == " " and (
            sourcecode[pos - 1] == " " or sourcecode[last] == "\n"
        ):
            # print("one further")
            pos = pos + 1
            continue
    ...
    if (
        endfound < 0 and startfound < 0
    ):  # and not "#" in badpart and not '"""' in badpart and not "'''" in badpart:
        #    print(sourcecode)
        #    print(":::::::::::")
        #    print(badpart)
        #    print("-----------------")
...
def getcontextPos(sourcecode, focus, fulllength):
    """
    Given a source code string, a focus position, and a desired context length,
    this function expands outwards from the focus position to find a context window
    (as a [start, end] index pair) that is at least 'fulllength' characters long.
    The expansion is done by moving to the previous or next split character (such as whitespace or punctuation)
    alternately, to ensure the context window is aligned with code boundaries.
    Returns None if a suitable window cannot be found.
    """
    startcontext = focus
    endcontext = focus
    ...
    while not len(sourcecode[startcontext:endcontext]) > fulllength:
        ...
        if (
            previoussplit(sourcecode, startcontext) == -1
            and nextsplit(sourcecode, endcontext) == -1
        ):
            #   print("NONE!")
            return None
...
def getblocks(sourcecode, badpositions, step, fulllength):
    """
    Splits the source code into blocks of a given length, labels each block as vulnerable (0) or not (1)
    based on overlap with badpositions, and avoids duplicate blocks.
    """
    blocks = []
        ...
        if not (focusarea == "\n"):
            ...
            if context is not None:
                ...
                for bad in badpositions:
                    if (
                        (context[0] > bad[0] and context[0] <= bad[1])
                        or (context[1] > bad[0] and context[1] <= bad[1])
                        or (context[0] <= bad[0] and context[1] >= bad[1])
                    ):
                        vulnerablePos = True
...
def getBadpart(change):
    ...
    for l in range(len(lines)):
        ...
        if len(line.replace(" ", "")) > 1:
            if line[0] == "-":
                if "#" not in line[1:].lstrip()[:3] and "import os" not in line:
...
def getIdentifiers(mode, nr):
    ...
    if mode == "remote_code_execution":
        if nr == "1":
            ...
            myfile = (
                "/src/main/python/monitoring_config_generator/yaml_tools/readers.py"
            )
        elif nr == "3":
